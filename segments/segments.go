...
// SegmentTrainingV5 documentation, this holds the end-point to API definitions
func SegmentTrainingV5(w http.ResponseWriter, r *http.Request) {
	...
	file, err := ioutil.ReadFile("json/segment_training_v5.csv")
	if err != nil {
		fmt.Fprintf(w, "Error reading segment_training_v5.csv - %s", err)
		w.WriteHeader(http.StatusNotFound)
	}
	w.Write(file)
	w.WriteHeader(http.StatusOK)
...
// SegmentEvaluationV5 documentation, this holds the end-point to API definitions
func SegmentEvaluationV5(w http.ResponseWriter, r *http.Request) {
	...
	file, err := ioutil.ReadFile("json/segment_evaluation_v5.csv")
	if err != nil {
		fmt.Fprintf(w, "Error reading segment_evaluation_v5.csv - %s", err)
		w.WriteHeader(http.StatusNotFound)
	}
	w.Write(file)
	w.WriteHeader(http.StatusOK)
